...
def get_skills_analysis():
    ...
    return jsonify(skills_analysis), 200
...
def analyze_skills(resume):
    ...
    1. Strong skills (list)
    2. Skills to improve (list)
    3. Recommended career paths (list)
    ...
    Format the response as a JSON object with the following keys:
    skills (object with 'strong' and 'improve' arrays), careerPath (array), suggestions (array)
    """
    ...
    try:
        ...
        # Ensure all required fields are present
        if 'skills' not in skills_analysis:
            skills_analysis['skills'] = {'strong': [], 'improve': []}
        if 'careerPath' not in skills_analysis:
            skills_analysis['careerPath'] = []
        if 'suggestions' not in skills_analysis:
            skills_analysis['suggestions'] = []
        
        return skills_analysis
    except Exception as e:
        print(f"Error parsing Gemini response for skills analysis: {e}")
        # Return default values if parsing fails
        return {
            'skills': {
                'strong': [],
            ...
            'careerPath': [],
            'suggestions': ["Error analyzing skills"]
        }
